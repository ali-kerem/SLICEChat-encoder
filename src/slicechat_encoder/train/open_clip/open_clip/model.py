...
from ....models.utils.factory import make_encoder_from_config
...
def _build_vision_tower(
        embed_dim: int,
        vision_cfg: Union[CLIPVisionCfg, dict],
        quick_gelu: bool = False,
        cast_dtype: Optional[torch.dtype] = None,
        grad_checkpointing: bool = False
):
    if isinstance(vision_cfg, dict):
        if "dilated_ratio" in vision_cfg.keys() or "d_state" in vision_cfg.keys(): # Handle LongNet and Mamba models. The rest is not modified.
            visual = make_encoder_from_config(vision_cfg, grad_checkpointing=grad_checkpointing)
            visual = visual.to(cast_dtype)
            return visual
        else:
...
class CustomTextCLIP(nn.Module):
    ...
    def __init__(
            ...
            embed_dim: int,
            vision_cfg: Union[CLIPVisionCfg, dict],
            text_cfg: Union[CLIPTextCfg, dict],
            quick_gelu: bool = False,
            init_logit_scale: float = np.log(1 / 0.07),
            init_logit_bias: Optional[float] = None,
            nonscalar_logit_scale: bool = False,
        ...
        super().__init__()
        self.output_dict = output_dict
        self.token_compression = vision_cfg.get("token_compression", None)
        self.visual = _build_vision_tower(embed_dim, vision_cfg, quick_gelu, cast_dtype, grad_checkpointing)
    ...
    def encode_image(self, normalize: bool = False, return_pooled_output: bool = True, **batch):
        output_dict = self.visual(
            token_embeddings=batch["features"], 
            positions=batch["coords"], 
            encoder_padding_mask=batch["padding_masks"],
            return_pooled_output=return_pooled_output
        )

        return_dict = {
            "encoder_out": output_dict['encoder_out'],
            "padding_mask": output_dict['padding_mask'],
            "positions": output_dict['positions'],
        }
        
        if return_pooled_output:
            pooled_out = output_dict['pooled_out']

            return_dict["pooled_out"] = F.normalize(pooled_out, dim=-1) if normalize else pooled_out

        if self.token_compression == "cropr":
            cropr_preds = output_dict['cropr_preds']
            if normalize:
                for i in range(len(cropr_preds)):
                    cropr_preds[i] = F.normalize(cropr_preds[i], dim=-1)
    ...
    def forward(
        ...
        image_encoder_out = self.encode_image(normalize=True, **batch)
        text_features = self.encode_text(batch["texts"], normalize=True)
        image_features = image_encoder_out['pooled_out']
        ...
        if self.output_dict:
            ...
            if self.logit_bias is not None:
                out_dict['logit_bias'] = self.logit_bias
            if self.token_compression == "cropr":
                out_dict['cropr_preds'] = image_encoder_out['cropr_preds']
            return out_dict

        if self.logit_bias is not None:
            if self.token_compression == "cropr":
                return image_features, text_features, self.logit_scale.exp(), self.logit_bias, image_encoder_out['cropr_preds']
            else:
                return image_features, text_features, self.logit_scale.exp(), self.logit_bias
        if self.token_compression == "cropr":
            return image_features, text_features, self.logit_scale.exp(), image_encoder_out['cropr_preds']
        else:
            return image_features, text_features, self.logit_scale.exp()
