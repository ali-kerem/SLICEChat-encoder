...
class ClipLoss(nn.Module):
    ...
    def forward(
        ...
        return_dict = {"contrastive_loss": total_loss}
        if cropr_preds is not None:
            cropr_loss = torch.tensor(0., device=device, dtype=total_loss.dtype)
            for i in range(len(cropr_preds)):
                logits_per_image, logits_per_text = self.get_logits(cropr_preds[i], text_features, logit_scale)
            ...
            return_dict["cropr_loss"] = cropr_loss

        return return_dict if output_dict else total_loss
