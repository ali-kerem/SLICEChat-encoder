...
def get_tokenizer(
    ...
    if model_name.startswith(HF_HUB_PREFIX):
        ...
        log_dir = os.path.dirname(os.path.dirname(resume))
        model_cfg_path = os.path.join(log_dir, "model_cfg.yaml")
        with open(model_cfg_path, 'r') as f:
            config = yaml.safe_load(f)
        if "text_cfg" not in config: # If we are doing pruner-training from a MAE checkpoint, we need to get the tokenizer from model config
...
def create_model(
        ...
        resume: Optional[str] = None,
        pruner_train: Optional[List[str]] = None,
        pruner_train_pooler_type: Optional[str] = None,
        pruner_train_cropr_cfg: Optional[Dict[str, Any]] = None,
        pruner_train_pruning_rate: Optional[float] = None,
        **model_kwargs,
):
    ...
    if resume:
        ...
        if "encoder_config" in model_cfg: # MAE to OpenCLIP conversion
            openclip_model_cfg = get_model_config(model_name)
            openclip_model_cfg["vision_cfg"] = model_cfg["encoder_config"]
            model_cfg = openclip_model_cfg

        if pruner_train_pooler_type is not None and pruner_train_pooler_type != "unchanged":
            if pruner_train_pooler_type != model_cfg["vision_cfg"]["pooler"]:
                logging.warning(f"WARNING: Pooler type in model config ({model_cfg['vision_cfg']['pooler']}) will be overridden by pruner-training to {pruner_train_pooler_type}.")
            model_cfg["vision_cfg"]["pooler"] = pruner_train_pooler_type
        
        if pruner_train_cropr_cfg is not None:
            model_cfg["vision_cfg"]["token_compression"] = "cropr"
            with open(pruner_train_cropr_cfg, 'r') as f:
                cropr_cfg = yaml.safe_load(f)
                model_cfg["vision_cfg"]["cropr_cfg"] = cropr_cfg
        
        if pruner_train_pruning_rate is not None:
            model_cfg["vision_cfg"]["pruning_rate"] = pruner_train_pruning_rate
    else:
    ...
    assert all(a in model_cfg for a in ('embed_dim', 'vision_cfg', 'text_cfg')), f"Model config {model_name} is missing required fields"
    ...
    if custom_text:
        if "multimodal_cfg" in model_cfg:
            ...
            model = CustomTextCLIP(**model_cfg, cast_dtype=cast_dtype, grad_checkpointing=grad_checkpointing)
    else:
    ...
    if pruner_train is not None:
        for name, param in model.named_parameters():
            if any(component in name for component in pruner_train):
...
def create_model_and_transforms(
    ...
    model, model_cfg = create_model(
        ...
        resume=resume,
        pruner_train=pruner_train,
        pruner_train_pooler_type=pruner_train_pooler_type,
        pruner_train_cropr_cfg=pruner_train_cropr_cfg,
        pruner_train_pruning_rate=pruner_train_pruning_rate,
        **model_kwargs,
    )
...
def convert_ckpt_to_customtextclip(ckpt_path: str) -> Dict[str, Any]: # TODO: Unify with convert_ckpt_for_pruner_train
    """Convert a checkpoint to CustomTextCLIP format by removing module prefix.
    
    This function loads a checkpoint and creates a new state dict with
    'module.' prefix removed from all parameter names.
    
    Args:
        ckpt_path: Path to the checkpoint file to convert.
    
    Returns:
        Dictionary containing the converted state dict with 'state_dict' key
        mapping to the parameters with module prefix removed.
    """
    ckpt = torch.load(ckpt_path)
    new_ckpt = {"state_dict": {}}
    for key, value in ckpt["state_dict"].items():
        if 'module.' in key:
            new_ckpt["state_dict"][key[len("module."):]] = value
    return new_ckpt


def create_customtextclip_and_tokenizer_from_ckpt(ckpt_path: str, device: str = "cpu", dtype: str = "bf16") -> Tuple[torch.nn.Module, Any]:
    """Load CustomTextCLIP model and tokenizer from checkpoint and config.
    
    This function loads a CustomTextCLIP model by reading the model configuration,
    creating the model architecture, loading checkpoint weights, and initializing
    the corresponding tokenizer from Hugging Face.
    
    Args:
        ckpt_path: Path to the checkpoint file. The model config is expected
            to be located at '../model_cfg.yaml' relative to the checkpoint.
        device: PyTorch device to load the model on.
    
    Returns:
        Tuple containing:
        - CustomTextCLIP model loaded with checkpoint weights, set to evaluation
          mode and cast to given dtype
        - Tokenizer loaded from Hugging Face based on config specifications
    """
    log_dir = "/".join(ckpt_path.rstrip("/").split("/")[:-2])

    model_config_path = os.path.join(log_dir, "model_cfg.yaml")
    model_config = yaml.safe_load(open(model_config_path))

    model = CustomTextCLIP(**model_config, cast_dtype=get_cast_dtype(dtype), grad_checkpointing=False, output_dict=True)
    model.load_state_dict(convert_ckpt_to_customtextclip(ckpt_path)["state_dict"])
    model = model.to(device=device, dtype=get_cast_dtype(dtype))
    ...
    tokenizer = get_tokenizer(f"hf-hub:{model_config['text_cfg']['hf_tokenizer_name']}", context_length=context_length)

    return model, tokenizer


def convert_openclip_ckpt_to_vision_tower(ckpt_path: str) -> Dict[str, Any]:
    """Convert a checkpoint to a format that can be used to create a Vision Tower model OpenCLIP format by extracting 
    visual module parameters.
    
    This function loads a checkpoint and creates a new state dict containing only
    the visual module parameters with 'module.visual.' prefix removed.
    ...
    Returns:
        Dictionary containing the converted state dict with 'state_dict' key
        mapping to the filtered parameters for Vision Tower model.
    """
    ckpt = torch.load(ckpt_path)
    new_ckpt = {}
    ...
    for key, value in ckpt["state_dict"].items():
        if 'module.visual.' in key:
            new_ckpt[key.replace("module.visual.", "")] = value

    return new_ckpt


def convert_ckpt_for_pruner_train(ckpt: Dict[str, Any]) -> Dict[str, Any]: # TODO: Unify with convert_ckpt_to_customtextclip
    """Convert a checkpoint to a format that can be used to pruner-train a model.
    
    Args:
        ckpt: State dict of the checkpoint to convert.

    Returns:
        Dictionary containing the converted state dict with 'state_dict' key
        mapping to the filtered parameters for pruner-training.
    """

    if "state_dict" in ckpt: # OpenCLIP
        new_ckpt = {k[len('module.'):]: v for k, v in ckpt["state_dict"].items()} # Remove module prefix
    elif "model" in ckpt: # MAE
        new_ckpt = {k.replace("encoder", "visual", 1): v for k, v in ckpt["model"].items() if k.startswith("encoder.")} # Replace encoder with visual
    else:
